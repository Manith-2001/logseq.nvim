...
describe('view.open_all buffer behavior (M6.3)', function()
  ...
  it('jump drills down into the local explorer, existing and dangling', function()
    ...
    view.jump()
    H.track_current()
    -- The overview wiped itself (bufhidden=wipe); a local explorer took
    -- the window, centered on the drilled-down page.
    local cur = vim.api.nvim_get_current_buf()
    assert.are.equal('logseq-graph', vim.bo[cur].filetype)
    local st = vim.api.nvim_buf_get_var(cur, 'logseq_graph')
    assert.are.equal('local', st.kind)
    assert.are.equal('B', st.title)
    assert.is_true(H.contains(cur, '## Incoming (1)'))
    -- Dangling rows drill down too, without creating the file.
    ...
    local st2 = vim.api.nvim_buf_get_var(cur2, 'logseq_graph')
    assert.are.equal('local', st2.kind)
    assert.are.equal('Missing M63', st2.title)
    assert.are.equal(0, vim.fn.filereadable(root .. '/pages/Missing M63.md'))
  end)
